...
// NewNewArchiver initializes a new archiver. When ctx is cancelled, all worker
// pools are stopped.
func NewNewArchiver(ctx context.Context, repo restic.Repository, fs fs.FS) *NewArchiver {
	blobSaver := NewBlobSaver(ctx, repo, defaultWorkers)
	fileSaver := NewFileSaver(ctx, fs, blobSaver, repo.Config().ChunkerPolynomial, 2)
	...
	fileSaver.NodeFromFileInfo = arch.nodeFromFileInfo
	return arch
}
...
func (arch *NewArchiver) Save(ctx context.Context, snPath, target string, previous *restic.Node) (fn FutureNode, excluded bool, err error) {
	fn = FutureNode{
		snPath: snPath,
		target: target,
	}
	...
	if errOpen != nil {
		// test if the open failed because target is a symbolic link
		if e, ok := errOpen.(*os.PathError); ok && e.Err == syscall.ELOOP {
	...
	case fs.IsRegularFile(fi):
		...
		if previous != nil && !previous.IsNewer(target, fi) {
			...
			arch.CompleteItem(snPath, previous, previous, ItemStats{})
			fn.node = previous
			_ = file.Close()
			return fn, false, nil
		}
		...
		fn.file = arch.fileSaver.Save(ctx, snPath, file, fi, func(node *restic.Node, stats ItemStats) {
			arch.CompleteItem(snPath, previous, node, stats)
		})
	...
	case fi.IsDir():
		...
		if err == nil {
			arch.CompleteItem(snItem, previous, fn.node, fn.stats)
		} else {
			_ = file.Close()
			return FutureNode{}, false, err
		}
