...
func saveFile(t testing.TB, repo restic.Repository, filename string, filesystem fs.FS) (*restic.Node, ItemStats) {
	...
	arch := NewNewArchiver(ctx, repo, filesystem)
	...
	var (
		callbackNode  *restic.Node
		callbackStats ItemStats
		callback      bool
	)
	...
	res := arch.fileSaver.Save(ctx, "/", file, fi, complete)
	if res.Err() != nil {
		t.Fatal(res.Err())
	}
...
func BenchmarkNewArchiverSaveFileSmall(b *testing.B) {
...
func BenchmarkNewArchiverSaveFileLarge(b *testing.B) {
...
func TestNewArchiverSaveDir(t *testing.T) {
	...
	for _, test := range tests {
		t.Run("", func(t *testing.T) {
			...
			arch := NewNewArchiver(ctx, repo, fs.Track{fs.Local{}})
...
func TestNewArchiverSaveTree(t *testing.T) {
	...
	for _, test := range tests {
		t.Run("", func(t *testing.T) {
			...
			arch := NewNewArchiver(ctx, repo, testFS)
