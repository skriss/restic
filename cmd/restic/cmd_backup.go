...
func runBackup(opts BackupOptions, gopts GlobalOptions, args []string) error {
	...
	warnError := func(item string, fi os.FileInfo, err error) error {
		Warnf("error for %v: %v\n", item, err)
		return nil
	}
	...
	arch.Error = warnError

	var completeStats archiver.ScanStats
	...
	go func() {
		...
		s, err := sc.Scan(gopts.ctx, target)
		if err != nil {
			panic(err)
		}

		term.Printf("scan done in %v, final result: %v files, %v dirs, %v bytes",
	...
	arch.StartItem = func(item string) {
		...
		complete := fmt.Sprintf("%v files, %v dirs, %v bytes",
			completeStats.Files,
			completeStats.Dirs,
			formatBytes(completeStats.Bytes),
		)

		term.SetStatus(append([]string{"", status, complete}, cur...))
	}
	...
	uploader := archiver.IndexUploader{
		Repository: repo,
		Start: func() {
			term.Printf("uploading index")
		},
		Complete: func(id restic.ID) {
			term.Printf("uploaded index %v", id.Str())
		},
	}

	var uploadWg sync.WaitGroup
	shutdown, shutdownCancel := context.WithCancel(gopts.ctx)

	var uploadError error

	uploadWg.Add(1)
	go func() {
		uploadError = uploader.Upload(gopts.ctx, shutdown, 30*time.Second)
		uploadWg.Done()
	}()
